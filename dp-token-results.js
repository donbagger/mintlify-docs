...
style.textContent = '.dp-tokens-table{width:100%!important;border-collapse:collapse!important;margin-top:20px!important;background:white!important;border-radius:8px!important;overflow:hidden!important;box-shadow:0 2px 8px rgba(0,0,0,0.1)!important}.dp-tokens-table th{background:#f8fafc!important;padding:12px 16px!important;text-align:left!important;font-weight:600!important;color:#374151!important;border-bottom:1px solid #e5e7eb!important}.dp-tokens-table td{padding:12px 16px!important;border-bottom:1px solid #f3f4f6!important;vertical-align:middle!important}.dp-tokens-table tr:hover{background:#f9fafb!important}.dp-token-symbol{font-weight:600!important;color:#16A34A!important}.dp-chain-badge{padding:4px 8px!important;border-radius:12px!important;font-size:12px!important;font-weight:500!important;text-transform:uppercase!important}.dp-chain-ethereum{background:#6366f1!important;color:white!important}.dp-chain-solana{background:#06b6d4!important;color:white!important}.dp-actions{display:flex!important;gap:8px!important;flex-wrap:wrap!important}.dp-actions a{padding:6px 12px!important;background:#16A34A!important;color:white!important;text-decoration:none!important;border-radius:4px!important;font-size:12px!important;font-weight:500!important;transition:background 0.2s!important}.dp-actions a:hover{background:#15803D!important}.dp-loading{text-align:center!important;color:#6b7280!important;font-style:italic!important}.dp-no-results{text-align:center!important;color:#6b7280!important;padding:40px 20px!important}.dp-results-header{margin-bottom:20px!important}.dp-results-header h3{margin:0 0 8px 0!important;color:#111827!important}.dp-results-header p{margin:0!important;color:#6b7280!important;font-size:14px!important}.dp-debug-results{position:fixed!important;top:50px!important;right:10px!important;background:#3b82f6!important;color:white!important;border:none!important;border-radius:4px!important;padding:4px 8px!important;font-size:12px!important;cursor:pointer!important;z-index:100000!important}';
...
function fetchTokenPrice(token) {
  return new Promise(function(resolve, reject) {
    try {
      ...
      fetch('https://api.dexpaprika.com/networks/' + token.chain + '/tokens/' + token.address)
...
function handleTokenLookupResults(urlParams) {
  try {
    ...
    var resultsContainer = document.getElementById('dp-token-results');
    var searchQueryElement = document.getElementById('dp-search-query');
    var tableBody = document.getElementById('dp-results-table-body');
    var noResultsElement = document.getElementById('dp-no-results');
    ...
    for (var i = 0; i < urlParams.entries().length; i++) {
      var entry = urlParams.entries()[i];
      var key = entry[0];
      var value = entry[1];
      if (key !== 'query' && value.includes(':')) {
        ...
        for (var j = 0; j < tokenData.length; j++) {
          var token = tokenData[j];
          if (token.chain === chain && token.address === address) {
            tokens.push(token);
            break;
          }
        }
    ...
    var promises = tokens.map(function(token) {
      return fetchTokenPrice(token).then(function(priceData) {
        return { token: token, priceData: priceData };
      });
    });
    ...
    Promise.all(promises).then(function(results) {
      ...
      for (var i = 0; i < results.length; i++) {
        ...
        row.innerHTML = '<td><span class="dp-token-symbol">' + token.symbol + '</span></td>' +
                       '<td>' + token.name + '</td>' +
                       '<td><span class="dp-chain-badge dp-chain-' + token.chain + '">' + token.chain + '</span></td>' +
                       '<td>$' + (priceData && priceData.price_usd ? priceData.price_usd : 'N/A') + '</td>' +
                       '<td>$' + (priceData && priceData.volume_usd_24h ? priceData.volume_usd_24h : 'N/A') + '</td>' +
                       '<td><div class="dp-actions">' +
